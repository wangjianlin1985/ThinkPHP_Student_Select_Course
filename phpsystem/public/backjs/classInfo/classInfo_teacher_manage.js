...
$(function () { 
	...
	$("#classInfo_manage").datagrid({
		url : backURL + getVisitPath("ClassInfo") + '/backTeacherList',
		fit : true,
		fitColumns : true,
		striped : true,
		rownumbers : true,
...
function initClassInfoManageTool() {
	classInfo_manage_tool = {
		init: function() {
			$.ajax({
				url : backURL + getVisitPath("SpecialInfo") + "/listAll",
				type : "post",
				dataType: "json",
						...
						if (classInfo) { 
							...
							$("#classInfo_classSpecialFieldNumber_specialFieldNumber_edit").combobox({
						        ...
						        onLoadSuccess: function () { //数据加载完毕事件
									$("#classInfo_classSpecialFieldNumber_specialFieldNumber_edit").combobox("select", classInfo.classSpecialFieldNumber);
