...
$(function () { 
	...
	$("#news_manage").datagrid({
		url : backURL + getVisitPath("News") + '/backList',
		fit : true,
		fitColumns : true,
		striped : true,
		rownumbers : true,
			...
			handler : function () {
				if ($("#newsEditForm").form("validate")) {
					...
					if(!$("#newsEditForm").form("validate")) {
						...
						$("#newsEditForm").form({
						    url: backURL + getVisitPath("News") + "/update",
						    onSubmit: function(){
								if($("#newsEditForm").form("validate"))  {
				                	$.messager.progress({
										text : "正在提交数据中...",
...
function initNewsManageTool() {
	news_manage_tool = {
		...
		exportExcel: function() {
			$("#newsQueryForm").form({
			    url: backURL + getVisitPath("News") + "/outToExcel",
			});
			//提交表单
			...
			if (rows.length > 0) {
				$.messager.confirm("确定操作", "您正在要删除所选的记录吗？", function (flag) {
					if (flag) {
						...
						$.ajax({
							type : "POST",
							url :  backURL + getVisitPath("News") + "/deletes",
							data : {
				...
				$.ajax({
					...
					success : function (news, response, status) {
						...
						if (news) { 
							...
							$("#news_newsPhotoImg").attr("src", publicURL + news.newsPhoto);
